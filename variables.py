...
kd=2*np.sqrt(np.vdot(K,K))*np.sin(theta/2)
k1=np.array([0,-1])
...
names_dic_Moire={'KM':str((0,0)),'GammaM':str(GammaM),'MM':str(MM)}
names_reversed_Moire=dict(zip(names_dic.values(),names_dic.keys()))
...
var_name_dict_Moire={r'$A$':tuple(A),r'$B$':tuple(B),r'$C$':tuple(C),r'$D$':tuple(D)}
var_dict_reversed_Moire=dict(zip(var_name_dict.values(),var_name_dict.keys()))
var_greek_dict_Moire={tuple(B):r'$(0,0)$',tuple(A):r'2$|k_{D}|\sin\theta(0,-1)$',tuple(C):r'$2|k_{D}|\sin(\theta)(0,-1)$',tuple(D):r'$2|k_{D}|\sin(\theta)(\sqrt{3}/2,-1/2)$'}
var_file_Moire={'A':tuple(A),'B':tuple(B),'C':tuple(C),'D':tuple(D)}
var_file_reversed_Moire=dict(zip(var_file.values(),var_file.keys()))
var_file_reversed_Moire=dict(zip(var_file.values(),var_file.keys()))
